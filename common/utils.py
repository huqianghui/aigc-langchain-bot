...
from langchain.tools import BaseTool
from .bing_search_multi_market import BingSearchAPIMultiMarketWrapper
...
try:
    from .prompts import (COMBINE_QUESTION_PROMPT, COMBINE_PROMPT, COMBINE_CHAT_PROMPT,
                          CSV_PROMPT_PREFIX, CSV_PROMPT_SUFFIX, MYSQL_AGENT_PREFIX, MYSQL_AGENT_FORMAT_INSTRUCTIONS,MSSQL_PROMPT, MSSQL_AGENT_PREFIX, 
...
def get_search_results(query: str, indexes: list, 
    ...
    for index in indexes:
        ...
        if vector_search:
            ...
            search_payload["select"]= "id, content, category, sourcefile"
        else:
            search_payload["select"]= "id, content, category, sourcefile"
        ...
        for result in search_results['value']:
            if result['@search.rerankerScore'] > reranker_threshold: # Show results that are at least N% of the max possible score=4
                content[result['id']]={
                                        ...
                                        "sourcefile": result['sourcefile'] + sas_token if result['sourcefile'] else "",
                                        "caption": result['@search.captions'][0]['text'],
                                        "score": result['@search.score'], # Uses the reranker score
                                        "index": index
                                    }
                # if vector_search:
                #     content[result['id']]["content"]= result['content']
                #     content[result['id']]["category"]= result['category']
                #     content[result['id']]["sourcefile"]= result['sourcefile']
...
def convert_language_to_code(language: str) -> str:
...
def convert_search_results_to_docs(ordered_search_results: OrderedDict, verbose: bool = False) -> List[Document]:
    document_list = []
    for index, content_data in ordered_search_results.items():
        document = Document(page_content=content_data["content"], metadata={
            "category": content_data["category"],
            "source": content_data["sourcefile"],
...
class DocSearchResults(BaseTool):
    ...
    description = "search documents in search engine about 科大讯飞"
...
class DocSearchTool(BaseTool):
    ...
    def _run(self, tool_input: Union[str, Dict],) -> str:
        try:
            docSearchResults = DocSearchResults(indexes=self.indexes,vector_only_indexes=self.vector_only_indexes,
            ...
            ordered_content = docSearchResults.run(parsed_input)
            
            for i in range(2):
                try:
                    response = get_answer(llm=self.llm,
                                            docs=convert_search_results_to_docs(ordered_content), 
                                            query=parsed_input, 
                                            language=detect_language(parsed_input), 
                                            chain_type="stuff",
                                            memory= None,
...
class SQLDbTool(BaseTool):
    ...
    def _run(self, query: str) -> str:
        ...
        agent_executor = create_sql_agent(
            prefix=MYSQL_AGENT_PREFIX,
            format_instructions = MYSQL_AGENT_FORMAT_INSTRUCTIONS,
            llm=self.llm,
            toolkit=toolkit,
            callback_manager=self.callbacks,
            top_k=self.k,
...
class BingSearchResults(BaseTool):
    ...
    def _run(self, query: str) -> str:
        # 判断query的语言，选择对应bing market
        market=convert_language_to_code(detect_language(query))
        bing = BingSearchAPIMultiMarketWrapper(k=self.k,market=market)
        try:
