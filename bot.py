...
from common.prompts import WELCOME_MESSAGE, CUSTOM_CHATBOT_PREFIX, CUSTOM_CHATBOT_SUFFIX
...
class MyBot(ActivityHandler):
    ...
    async def on_message_activity(self, turn_context: TurnContext):
        ...
        book_search = DocSearchTool(llm=llm, vector_only_indexes = vector_only_indexes,
                           ...
                           verbose=True)
        www_search = BingSearchTool(llm=llm, k=3, callback_manager=cb_manager, return_direct=True,verbose=True)
        sql_search = SQLDbTool(llm=llm, k=3, callback_manager=cb_manager, return_direct=True,verbose=True)
        chatgpt_search = ChatGPTTool(llm=llm, callback_manager=cb_manager, return_direct=True,verbose=True)
        ...
        agent = ConversationalChatAgent.from_llm_and_tools(llm=llm_a, tools=tools, system_message=CUSTOM_CHATBOT_PREFIX, human_message=CUSTOM_CHATBOT_SUFFIX,verbose=True)
        agent_chain = AgentExecutor.from_agent_and_tools(agent=agent, tools=tools, memory=self.memory,verbose=True)
